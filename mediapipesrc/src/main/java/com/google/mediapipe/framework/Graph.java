...
//      logger.atInfo().log("MediaPipe graph won't start until all stream headers are available.");
      Log.i(Graph.class.getSimpleName(), "MediaPipe graph won't start until all stream headers are available.");
      return;
    }
...
//          logger.atSevere().log("Stream: %s might be missing.", entry.getKey());
          Log.i(Graph.class.getSimpleName(), "Stream: " + entry.getKey() + " might be missing.");
        }
      }
...
//                "AddPacket for stream: %s failed: %s.", entry.getKey(), e.getMessage());
            Log.e(Graph.class.getSimpleName(), "AddPacket for stream: " + entry.getKey() + " failed: " + e.getMessage());
            throw e;
          }
