...
//      logger.atSevere().withCause(e).log("Unable to get files in assets path: %s", assetsPath);
      Log.e(AssetCache.class.getSimpleName(), "Unable to get files in assets path: " + assetsPath);
      e.printStackTrace();
    }
    if (assetFiles == null || assetFiles.length == 0) {
//      logger.atWarning().log("No files to load");
      Log.i(AssetCache.class.getSimpleName(), "No files to load");
      return;
    }
...
//      logger.atSevere().log("Unable to unpack: %s", assetPath);
      Log.e(AssetCache.class.getSimpleName(), "Unable to unpack: " + assetPath);
      ioe.printStackTrace();
      try {
        if (inStream != null) {
          inStream.close();
...
//      logger.atInfo().log("Current app version code: %d", appVersionCode);
      Log.i(AssetCache.class.getSimpleName(), "Current app version code: " + appVersionCode);
